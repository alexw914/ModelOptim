...
'''
将coco格式的json文件转换为yolo的txt格式。
params:
    json_file: coco格式的json文件路径
    out_path:  输出yolo格式txt的文件夹 
usage:
...
class COCO2YOLO:
    ...
    def __init__(self, json_file, out_path):
        ...
        self._check_file_and_dir(json_file, out_path)
    ...
    def _save_txt(self, anno_dict):
        for k, v in anno_dict.items():
            ...
            with open(os.path.join(self.out_path, file_name), 'w', encoding='utf-8') as f:
                print(k, v)
                for obj in v:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_file', help='JSON file', dest='json', required=True)
    parser.add_argument('--out_path', help='path to output folder', default="./labels", required=True)
    args = parser.parse_args()
    Path(args.out_path).mkdir(exist_ok=True)
    c2y = COCO2YOLO(args.json_file, args.out_path)
    c2y.coco2yolo()
