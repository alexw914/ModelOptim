...
def load_checkpoint(path, map_location='cpu', weights_only=None):
    ...
    try:
        ...
        if weights_only is False:
            raise
        print(
            "Default safe checkpoint load failed; retrying with weights_only=False "
            f"for trusted checkpoint: {path}"
        )
